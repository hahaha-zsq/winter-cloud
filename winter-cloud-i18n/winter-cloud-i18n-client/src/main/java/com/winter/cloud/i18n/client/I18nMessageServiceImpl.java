package com.winter.cloud.i18n.client;
 ...
 *
 * <p>
 * 该类实现了 {@link I18nMessageService} 接口，负责从多级缓存或数据库中获取国际化消息。
 * 采用了 <b>Redis 缓存 + 布隆过滤器 + 分布式锁</b> 的高可用架构，旨在解决高并发场景下的缓存穿透、击穿和雪崩问题。
 * </p>
 *
 * <h3>核心特性：</h3>
 * <ul>
 * <li><b>防穿透 (Bloom Filter)：</b> 使用布隆过滤器拦截数据库中不存在的 Key。</li>
 * <li><b>防击穿 (Distributed Lock)：</b> 使用 Redisson 互斥锁防止热点 Key 失效时引发的数据库风暴。</li>
 * <li><b>防雪崩 (Random TTL)：</b> 设置随机缓存过期时间，避免同一时刻大量缓存失效。</li>
 * <li><b>性能优化：</b> 包含双重检查锁 (DCL) 和自旋等待机制。</li>
 * </ul>
 *
 * <h3>请求处理详细流程图：</h3>
 * <pre>
 ...
 * +-------+--------+      不存在          +---------------------------+
 * | 2. 布隆过滤器    |-------------------> |  缓存空值 -> 返回默认值     |
 * | (Bloom Filter) |                      |  (防止恶意请求穿透到 DB)    |
 * +-------+--------+                      +---------------------------+
 * |
 * | 可能存在 (Go to DB)
 * v
 * +-------+--------+      获取锁超时       +---------------------------+
 * | 3. 获取分布式锁  |-------------------> |  自旋等待 (Spin Wait)      |
 * | (Redisson Lock)|                      |  重试读缓存 -> 返回/Null    |
 * +-------+--------+                      +---------------------------+
 * |
 * | 成功获取锁
 * v
 * +-------+--------+      有值 (被别人预热) +---------------------------+
 * | 4. 双重检查Redis |-------------------> |  释放锁 -> 返回缓存值       |
 * | (Double Check) |                      +---------------------------+
 * +-------+--------+
 * |
 * | 仍无值 (我是第一个)
 * v
 * +-------+--------+      有数据          +---------------------------+
 * | 5. 查询数据库    |-------------------> |  写入 Redis (随机过期时间)  |
 * | (Source of Truth)|                      |  释放锁 -> 返回数据        |
 * +-------+--------+                      +---------------------------+
 * |
 * | 无数据 (DB也不存在)
 * v
...
public class I18nMessageServiceImpl implements I18nMessageService {
    ...
    private final I18nMessageInfo i18nMessageInfo;
    // ==================== Winter I18n 框架接口实现 ====================
        ...
        try {
            ...
            if (lock.tryLock(3, CommonConstants.I18nMessage.I18N_LOCK_EXPIRE_SECONDS, TimeUnit.SECONDS)) {
                try {
                    ...
                    String message = i18nMessageInfo.findMessageByKeyAndLocale(messageKey, locale);
